...
static std::vector<loc_types::loc_entry_t> build_cel_entries(build_context_t & context)
{
	...
	for (const auto & record : it_cell->second.records)
	{
		...
		{
			app_logger_t::add_log("[warning] cel: cannot encode entry \"" + record.old_text + "\"\r\n");
...
static std::vector<loc_types::loc_entry_t> build_mrk_entries(build_context_t & context)
{
	...
	for (const auto & record : it_dial->second.records)
	{
		...
		if (!try_encode_entry(entry, context.codepage))
		{
			app_logger_t::add_log("[warning] mrk: cannot encode entry \"" + record.new_text + "\"\r\n");
			++context.skipped_count;
			continue;
		}
...
static void insert_top_entry(top_build_context_t & context, const loc_types::loc_entry_t & candidate)
{
...
static void process_top_record(top_build_context_t & context, const record_entry_t & record)
{
...
static std::vector<loc_types::loc_entry_t> build_top_entries(build_context_t & context, const top_input_t & top_input)
{
	const auto it_dial = context.dict.find(rec_type_t::dial);
	if (it_dial == context.dict.end())
		return {};
	...
	{
		inflection_available = inflector.load(top_input.aff_path, top_input.dic_path);
...
static std::string build_output_path(const std::string & output_directory, const std::string & filename)
{
...
loc_generator::generation_result_t loc_generator::generate(const generation_input_t & input)
{
	generation_result_t result;
	result.cel_path = build_output_path(input.output_directory, input.esm_name + ".cel");
	result.mrk_path = build_output_path(input.output_directory, input.esm_name + ".mrk");
	result.top_path = build_output_path(input.output_directory, input.esm_name + ".top");
	...
	app_logger_t::add_log(
	    "[info] loc generation complete: cel=" + std::to_string(result.cel_entries) +
	    " mrk=" + std::to_string(result.mrk_entries) + " top=" + std::to_string(result.top_entries) + " skipped=" +
	    std::to_string(result.skipped_entries) + " collisions=" + std::to_string(result.collision_warnings) + "\r\n");
