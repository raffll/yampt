...
void creator_single_t::run()
{
	...
	creator_helpers_t::build_text_match_index(m_ctx);
...
void creator_single_t::make_gmst()
{
	...
	for (size_t i = 0; i < m_ctx.esm.get_records().size(); ++i)
	{
		...
		if (m_with_base)
			creator_helpers_t::insert_entry_single_with_base(m_ctx, key_text, text, text, rec_type_t::gmst);
		else
			creator_helpers_t::insert_entry_single(m_ctx, key_text, text, text, rec_type_t::gmst);
	}
}
...
void creator_single_t::make_fnam()
{
	...
	for (size_t i = 0; i < m_ctx.esm.get_records().size(); ++i)
	{
		...
		if (m_with_base)
			creator_helpers_t::insert_entry_single_with_base(m_ctx, key_text, text, text, rec_type_t::fnam);
		else
			creator_helpers_t::insert_entry_single(m_ctx, key_text, text, text, rec_type_t::fnam);
	}
}
...
void creator_single_t::make_desc()
{
	...
	for (size_t i = 0; i < m_ctx.esm.get_records().size(); ++i)
	{
		...
		if (m_with_base)
			creator_helpers_t::insert_entry_single_with_base(m_ctx, key_text, text, text, rec_type_t::desc);
		else
			creator_helpers_t::insert_entry_single(m_ctx, key_text, text, text, rec_type_t::desc);
	}
}
...
void creator_single_t::make_text()
{
	...
	for (size_t i = 0; i < m_ctx.esm.get_records().size(); ++i)
	{
		...
		if (m_with_base)
			creator_helpers_t::insert_entry_single_with_base(m_ctx, key_text, text, text, rec_type_t::text);
		else
			creator_helpers_t::insert_entry_single(m_ctx, key_text, text, text, rec_type_t::text);
	}
}
...
void creator_single_t::make_rnam()
{
	...
	for (size_t i = 0; i < m_ctx.esm.get_records().size(); ++i)
	{
		...
		while (m_ctx.esm.get_value().exist)
		{
			...
			if (m_with_base)
				creator_helpers_t::insert_entry_single_with_base(m_ctx, key_text, text, text, rec_type_t::rnam);
			else
				creator_helpers_t::insert_entry_single(m_ctx, key_text, text, text, rec_type_t::rnam);
...
void creator_single_t::make_indx()
{
	...
	for (size_t i = 0; i < m_ctx.esm.get_records().size(); ++i)
	{
		...
		if (m_with_base)
			creator_helpers_t::insert_entry_single_with_base(m_ctx, key_text, text, text, rec_type_t::indx);
		else
			creator_helpers_t::insert_entry_single(m_ctx, key_text, text, text, rec_type_t::indx);
	}
}
...
void creator_single_t::make_info()
{
	...
	for (size_t i = 0; i < m_ctx.esm.get_records().size(); ++i)
	{
		...
		if (m_ctx.esm.get_record().id == "DIAL")
		{
			...
			if (m_ctx.esm.get_key().exist && m_ctx.esm.get_value().exist)
				key_prefix = domain_types_t::get_dialog_type(m_ctx.esm.get_key().content) + "^" +
		...
		if (m_with_base)
			creator_helpers_t::insert_entry_single_with_base(m_ctx, key_text, text, text, rec_type_t::info);
		else
			creator_helpers_t::insert_entry_single(m_ctx, key_text, text, text, rec_type_t::info);

		creator_helpers_t::enrich_info_speaker(m_ctx, key_text, i);
	}
}
...
void creator_single_t::make_sctx()
{
	...
	for (size_t i = 0; i < m_ctx.esm.get_records().size(); ++i)
	{
		...
		const auto & messages = creator_helpers_t::make_script_messages(m_ctx.esm.get_value().text);
		...
		for (size_t k = 0; k < messages.size(); ++k)
		{
			...
			if (m_with_base)
				creator_helpers_t::insert_entry_single_with_base(m_ctx, key_text, old_text, old_text, rec_type_t::sctx);
			else
				creator_helpers_t::insert_entry_single(m_ctx, key_text, old_text, old_text, rec_type_t::sctx);
		}
	}
}
...
void creator_single_t::make_bnam()
{
	...
	for (size_t i = 0; i < m_ctx.esm.get_records().size(); ++i)
	{
		...
		for (size_t k = 0; k < messages.size(); ++k)
		{
			...
			if (m_with_base)
				creator_helpers_t::insert_entry_single_with_base(m_ctx, key_text, old_text, old_text, rec_type_t::bnam);
			else
				creator_helpers_t::insert_entry_single(m_ctx, key_text, old_text, old_text, rec_type_t::bnam);
		}
	}
}
...
void creator_single_t::make_dial()
{
	...
	for (size_t i = 0; i < m_ctx.esm.get_records().size(); ++i)
	{
		...
		if (m_with_base)
			creator_helpers_t::insert_entry_single_with_base(m_ctx, text, text, text, rec_type_t::dial);
		else
			creator_helpers_t::insert_entry_single(m_ctx, text, text, text, rec_type_t::dial);
	}
}
...
void creator_single_t::make_cell()
{
	...
	for (size_t i = 0; i < m_ctx.esm.get_records().size(); ++i)
	{
		...
		if (m_with_base)
			creator_helpers_t::insert_entry_single_with_base(m_ctx, text, text, text, rec_type_t::cell);
		else
			creator_helpers_t::insert_entry_single(m_ctx, text, text, text, rec_type_t::cell);
	}
}
