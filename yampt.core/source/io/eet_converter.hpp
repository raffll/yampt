...
public:
	...
	const tools_t::dict_t & get_dict() const
	{
		return m_dict;
	}
...
private:
	tools_t::rec_type_t map_type(const std::string & rec_type, const std::string & sub_type) const;
	status_t map_status(uint8_t status_byte) const;
	std::string build_key_text(const eet_reader_t::eet_entry_t & entry, tools_t::rec_type_t yampt_type) const;

	tools_t::dict_t m_dict;
