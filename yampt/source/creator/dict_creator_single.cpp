    ...
    tools_t::rec_type_t type)
{
	...
	insert_duplicate(key_text, old_text, new_text, type, status_t::untranslated);
}
    ...
    tools_t::rec_type_t type)
{
	...
	if (!is_approved)
	{
		insert_unapproved_changed(key_text, old_text, base_entry, type);
		return;
	}

	if (differs_only_in_numbers_or_punct(old_text, base_entry.old_text))
	{
		insert_adapted_entry(key_text, old_text, base_entry, type);
		return;
	}

	insert_with_status(key_text, old_text, base_entry.new_text, type, status_t::changed);
    ...
    tools_t::rec_type_t type)
{
	...
	{
		insert_with_status(key_text, old_text, base_entry.new_text, type, status_t::outdated);
	...
	{
		insert_with_status(key_text, old_text, old_text, type, status_t::untranslated);
		return;
	}

	insert_with_status(key_text, old_text, old_text, type, status_t::changed);
    ...
    tools_t::rec_type_t type)
{
	...
	{
		insert_with_status(key_text, old_text, adapted, type, status_t::translated);
		return;
	}

	insert_with_status(key_text, old_text, adapted, type, status_t::adapted);
    ...
    tools_t::rec_type_t type)
{
	...
	auto it_chapter = base_dict->find(type);
	if (it_chapter == base_dict->end())
	{
	...
	{
		insert_with_status(key_text, old_text, base_entry->new_text, type, status_t::mismatch);
		return;
	}

	if (base_entry->old_text == old_text && base_entry->new_text == old_text)
	{
		insert_with_status(key_text, old_text, old_text, type, base_entry->status);
		return;
	}
	...
	insert_changed_entry(key_text, old_text, *base_entry, type);
}
    ...
    tools_t::rec_type_t type)
{
	...
	insert_duplicate(key_text, old_text, old_text, type, status_t::untranslated);
}
