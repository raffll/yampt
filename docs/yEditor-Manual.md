...
- **Merged Patch** — three sub-tabs control how auto-merge behaves:
  - **Exclude Sub-Records** — a list of sub-records excluded from conflict detection and the merged patch. Each entry uses `RECORD:SUB` format. By default it holds `CELL:NAM0` (a cell's object count, which is recalculated) and `LTEX:INTV` (a land texture's index, which is assigned per plugin and legitimately differs). Use `TYPE:*` to exclude an entire record type. Add entries via the input field or right-click a sub-record row in the record view and choose "Exclude Sub-Record."
