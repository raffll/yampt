...
apply keyed_list_merge to remaining list types (NPCO + NPCS DONE): the shared 3-way keyed list merge sub_record_merge_t::keyed_list_merge is implemented (rule: deletion is a modification-to-absent, highest-priority opinion wins, additions unioned; unit-tested). The generic sub_record_merge_t::merge_keyed_list_phase(input, first, winner, output, sub_type, key_of) routes any single-sub-record list through it. NPCO (key = item ID) and NPCS (key = 32-byte spell ID) are wired and excluded from positional apply_intermediate. Still to route through it:
  - leveled list entries (LEVI INAM+INTV / LEVC CNAM+INTV): ALREADY CORRECT — leveled_list_merge_t already keys by (id, level), respects deletion (is_item_deleted: any contributing version lacking an entry drops it, equivalent to our rule since a leveled entry has no modifiable data so "present==master" is never an opinion and delete wins over unchanged), and unions additions. It additionally keeps the MAX occurrence count across versions (leveled-specific, preserves probability weighting) — this is intended and has no analogue in the other list types. No rewiring needed; do NOT force it through the single-type merge_keyed_list_phase (would lose count handling).
  - faction reactions (FACT INTV+ANAM pairs): DONE — collect_faction_reactions collects ANAM+INTV pairs keyed by faction name (lowest-INTV dedup within a version, per OpenMW), runs keyed_list_merge, replace_faction_reactions rebuilds the pairs; ANAM/INTV are excluded from positional apply_intermediate for FACT. Fixes the reorder-drop, absent-vs-conflict, and partial-merge FACT reaction bugs.
  - ENAM effects: DO NOT key — positional/index-based merge (slots 0..7), absent slot = not overriding (exclude-empty), matching TES3Merge EffectList. Separate from keyed_list_merge.
  do one type at a time, keep existing tests green, add failing-first tests per type.
