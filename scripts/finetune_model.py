...
Input:  JSON dict files from scripts/finetune/
...
DICT_DIR = os.path.join(SCRIPT_DIR, "finetune")
MODEL_NAME = "facebook/nllb-200-distilled-600M"
OUTPUT_DIR = os.path.join(REPO_ROOT, "models", "nllb-600M", "model")
LORA_DIR = os.path.join(SCRIPT_DIR, "finetune_tmp", "lora")
MERGED_DIR = os.path.join(SCRIPT_DIR, "finetune_tmp", "merged")
SRC_LANG = "eng_Latn"
TGT_LANG = "pol_Latn"
MIN_LENGTH = 5
MAX_LENGTH = 128
...
def merge_and_convert():
    ...
    spm_src = os.path.join(MERGED_DIR, "sentencepiece.bpe.model")
    spm_dst = os.path.join(os.path.dirname(OUTPUT_DIR), "sentencepiece.bpe.model")
    if os.path.exists(spm_src) and not os.path.exists(spm_dst):
        import shutil
        shutil.copy2(spm_src, spm_dst)
        print(f"Copied sentencepiece.bpe.model to {spm_dst}")


def cleanup():
    import shutil

    tmp_dir = os.path.join(SCRIPT_DIR, "finetune_tmp")
...
def main():
    if not os.path.isdir(DICT_DIR):
        ...
        print("Place base dictionary JSON files in the scripts/finetune/ folder.")
        sys.exit(1)
