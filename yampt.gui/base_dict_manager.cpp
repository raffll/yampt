...
{
	merged_dict_ = tools_t::initialize_dict();

	if (paths_.empty())
		return;

	dict_merger_t merger(paths_);
	merged_dict_ = merger.get_dict();
}
...
auto_translate_result_t base_dict_manager_t::auto_translate_from_base(editor_state_t & state)
{
	...
	for (auto & [type, chapter] : state.get_user_dict())
	{
		...
		for (size_t i = 0; i < chapter.records.size(); ++i)
		{
			...
			bool is_untranslated = rec.status.empty() || rec.status == tools_t::status_t::untranslated;
			if (!is_untranslated)
				continue;
			...
			rec.status = tools_t::status_t::auto_identical;
...
auto_translate_result_t base_dict_manager_t::auto_translate_identical(editor_state_t & state)
{
	...
	for (const auto & [type, chapter] : state.get_user_dict())
	{
		...
		{
			bool is_translated = !rec.status.empty() && rec.status != tools_t::status_t::untranslated;
			if (!is_translated)
				continue;
	...
	for (auto & [type, chapter] : state.get_user_dict())
	{
		for (size_t i = 0; i < chapter.records.size(); ++i)
		{
			...
			for (const auto & [src_text, translated_text] : translated_pairs)
			{
				...
				rec.status = tools_t::status_t::auto_heuristic;
				state.mark_modified(type, i);
				++result.translated;
				matched = true;
				break;
