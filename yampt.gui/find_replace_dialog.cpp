    ...
    : QDialog(parent)
{
    ...
    connect(find_next_btn_, &QPushButton::clicked, this, [this]() {
        emit find_next_requested(find_field_->text(), case_check_->isChecked(), regex_check_->isChecked());
    });

    connect(replace_btn_, &QPushButton::clicked, this, [this]() {
        emit replace_requested(find_field_->text(), replace_field_->text(), case_check_->isChecked(), regex_check_->isChecked());
    });

    connect(replace_all_btn_, &QPushButton::clicked, this, [this]() {
        emit replace_all_requested(find_field_->text(), replace_field_->text(), case_check_->isChecked(), regex_check_->isChecked());
    });
}
