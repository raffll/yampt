...
std::vector<annotation_t> annotation_manager_t::annotate(const std::string & text,
    ...
    std::vector<annotation_t> & results) const
{
    for (const auto & [term, translated] : terms)
    {
        ...
        while ((pos = text_lower.find(term, pos)) != std::string::npos)
        {
            ...
            ann.kind = kind;
            ann.original = text_original.substr(pos, term.size());
            ann.translated = translated;
...
void annotation_manager_t::load_glossary(const std::string & path)
{
    ...
    const auto & dict = reader.getDict();
