...
std::vector<token_t> syntax_highlighter_t::tokenize(
...
std::vector<token_t> syntax_highlighter_t::tokenize_sctx(const std::string & text) const
{
    static const std::string keywords[] = {
        "messagebox", "say", "journal", "choice", "addtopic",
        "getpccell", "positioncell", "showmap", "centeroncell",
        "aifollowcell", "aiescortcell", "placeitemcell"
    };
    ...
    while (line_start <= text.size())
    {
        ...
        for (const auto & kw : keywords)
        {
            ...
            while (search_pos < line_end)
            {
                ...
                if (is_word_boundary(text, found, kw.size())
                    && !overlaps_any(tokens, found, found + kw.size()))
                {
    ...
    std::sort(tokens.begin(), tokens.end(), [](const token_t & a, const token_t & b)
    {
...
{
    static const std::string tag_names[] = {
        "div", "font", "br", "p", "img", "b"
    };
