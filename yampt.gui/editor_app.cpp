...
void editor_app_t::render_toolbar()
{
	...
	{
		ImGui::SetNextItemWidth(200.0f);
		bool replace_enter = ImGui::InputText(
		    "Replace", replace_buffer_.data(), replace_buffer_.size(), ImGuiInputTextFlags_EnterReturnsTrue);
		ImGui::SameLine();
...
void editor_app_t::render_text_with_topic_highlights(const std::string & text)
{
	...
	std::sort(
	    annotations.begin(),
	    annotations.end(),
	    [](const annotation_t & a, const annotation_t & b) { return a.start < b.start; });
	...
	for (const auto & ann : annotations)
	{
		...
		if (ImGui::IsItemHovered())
			ImGui::SetTooltip("%s", ann.new_text.c_str());
