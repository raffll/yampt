...
void search_engine_t::set_config(const config_t & cfg)
{
	config_ = cfg;
...
bool search_engine_t::matches(const table_row_t & row) const
{
	...
	const auto query = config_.case_sensitive ? config_.query : to_lower(config_.query);
	...
	for (const auto & col : config_.columns)
	{
		...
		const auto haystack = config_.case_sensitive ? *text : to_lower(*text);
