...
size_t plugin_cleaner_t::remove_itm_records(int plugin_idx)
{
	...
	const auto count_before = m_scan.active_record_count();

	for (const auto * entry : itms)
		m_scan.remove_from_active(entry->rec_type, entry->record_id);

	const auto removed = count_before - m_scan.active_record_count();
	if (removed > 0)
		m_log("[info] removed " + std::to_string(removed) + " ITM records from " + m_scan.plugin_filename(plugin_idx));
