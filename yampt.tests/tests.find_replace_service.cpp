...
#include "../yampt.translator/controller/find_replace_service.hpp"
...
class mock_row_provider_t : public row_provider_t
{
public:
...
TEST_CASE("find_replace_service_t::find_next, finds matching row", "[i][qt]")
{
	mock_row_provider_t provider;
	...
	row_second.status = "untranslated";
	row_second.record_index = 1;

	provider.rows = { row_first, row_second };
	...
	find_replace_service_t service(provider, active_doc);
...
TEST_CASE("find_replace_service_t::find_next, wraps around to first row", "[i][qt]")
{
	mock_row_provider_t provider;
...
TEST_CASE("find_replace_service_t::find_next, not found returns empty", "[i][qt]")
{
	mock_row_provider_t provider;
...
TEST_CASE("find_replace_service_t::find_next, case insensitive", "[i][qt]")
{
	mock_row_provider_t provider;
...
TEST_CASE("find_replace_service_t::replace_all, replaces all matching", "[i][qt]")
{
	mock_row_provider_t provider;
	...
	row_third.status = "untranslated";
	row_third.record_index = 2;

	provider.rows = { row_first, row_second, row_third };
...
TEST_CASE("find_replace_service_t::replace_all, sets status in_progress", "[i][qt]")
{
	mock_row_provider_t provider;
	...
	const auto & records = doc.data().at(tools_t::rec_type_t::cell).records;
	REQUIRE(records[0].status == "in_progress");
