...
TEST_CASE("dict_reader parses all new status values", "[u][reader]")
{
	...
	dict.at(tools_t::rec_type_t::dial).insert({ "dial_1", "topic", "temat", tools_t::status_t::info });
	dict.at(tools_t::rec_type_t::dial).insert({ "dial_2", "hello", "hello", tools_t::status_t::exact });
	dict.at(tools_t::rec_type_t::info).insert({ "info_1", "Hello", "Hello", tools_t::status_t::identical });
	dict.at(tools_t::rec_type_t::info).insert({ "info_2", "Goodbye", "Do widzenia", tools_t::status_t::translated });
	dict.at(tools_t::rec_type_t::info).insert({ "info_3", "Yes", "Tak", tools_t::status_t::reused });
	dict.at(tools_t::rec_type_t::info).insert({ "info_4", "No", "Nie", tools_t::status_t::adapted });
	dict.at(tools_t::rec_type_t::info).insert({ "info_5", "Maybe", "Może", tools_t::status_t::changed });
	dict.at(tools_t::rec_type_t::info).insert({ "info_6", "Quest", "Quest", tools_t::status_t::untranslated });
	dict.at(tools_t::rec_type_t::info).insert({ "info_7", "Done", "Gotowe", tools_t::status_t::translated });
	dict.at(tools_t::rec_type_t::info).insert({ "info_8", "Final", "Końcowy", tools_t::status_t::translated });
	dict.at(tools_t::rec_type_t::info).insert({ "info_9", "Bad", "Źle", tools_t::status_t::error });
	...
	REQUIRE(e10 != nullptr);
	REQUIRE(e10->status == tools_t::status_t::error);
}
...
TEST_CASE("dict_reader write-read round trip preserves all fields", "[u][reader]")
{
	...
	REQUIRE(info_entry->status == "translated");
	...
	REQUIRE(cell_entry->status == "coords");
	REQUIRE(cell_entry->speaker_name.empty());
}
