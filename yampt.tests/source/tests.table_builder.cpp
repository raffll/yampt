...
static dict_t make_single_entry_dict(
    rec_type_t type,
...
static table_build_result_t build_filtered_rows(
    const dict_t & data,
    const std::set<rec_type_t> & type_filter,
...
TEST_CASE("table_builder::build_filtered_rows, counts translated entries correctly", "[u]")
{
	...
	dict[rec_type_t::cell].records.push_back(entry_one);
	...
	dict[rec_type_t::cell].records.push_back(entry_two);
	...
	REQUIRE(result.counts.translated_counts.at(rec_type_t::cell) == 1);
...
TEST_CASE("table_builder::build_filtered_rows, progress counts respect type filter", "[u]")
{
	...
	dict[rec_type_t::fnam].records.push_back(fnam_entry);
...
TEST_CASE("table_builder::build_filtered_rows, bnam counts under info type", "[u]")
{
	...
	dict[rec_type_t::bnam].records.push_back(bnam_entry);
	...
	REQUIRE(result.counts.type_counts.count(rec_type_t::info) == 1);
...
TEST_CASE("table_builder::build_filtered_rows, bnam interleaved after matching info", "[u]")
{
	...
	dict[rec_type_t::info].records.push_back(info_entry);
	...
	REQUIRE(result.rows[0].type == rec_type_t::info);
	REQUIRE(result.rows[1].type == rec_type_t::bnam);
...
TEST_CASE("table_builder::build_filtered_rows, sub type counts for info records", "[u]")
{
	...
	dict[rec_type_t::info].records.push_back(topic_entry);

	record_entry_t voice_entry;
	...
	dict[rec_type_t::info].records.push_back(voice_entry);
...
TEST_CASE("table_builder::build_filtered_rows, sub type filter active when solo mode", "[u]")
{
	...
	row_filter_t search;
	std::set<rec_type_t> type_filter = { rec_type_t::info };
...
TEST_CASE("table_builder::build_filtered_rows, multiple types counted independently", "[u]")
{
	...
	dict[rec_type_t::gmst].records.push_back(gmst_entry);
