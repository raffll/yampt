...
{
	dict_merger_t merger;
	merger.add_record(tools_t::rec_type_t::cell, "Balmora", "First");
	merger.add_record(tools_t::rec_type_t::cell, "Balmora", "Second");
...
{
	dict_merger_t merger;
	merger.add_record(tools_t::rec_type_t::cell, "Balmora", "CellTranslation");
	merger.add_record(tools_t::rec_type_t::fnam, "Balmora", "FnamTranslation");

	const auto * cell_entry = merger.get_dict().at(tools_t::rec_type_t::cell).find("Balmora");
	const auto * fnam_entry = merger.get_dict().at(tools_t::rec_type_t::fnam).find("Balmora");
...
{
	dict_merger_t merger;
	REQUIRE(tools_t::get_number_of_elements_in_dict(merger.get_dict()) == 0);
}
