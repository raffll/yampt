...
TEST_CASE("compute_conflict_all, single value yields only_one", "[u]")
{
	...
	REQUIRE(record_conflict_t::compute_conflict_all(values) == conflict_all_t::only_one);
}
...
TEST_CASE("compute_conflict_all, all same yields no_conflict", "[u]")
{
	...
	REQUIRE(record_conflict_t::compute_conflict_all(values) == conflict_all_t::no_conflict);
}
...
TEST_CASE("compute_conflict_all, multiple differ yields conflict", "[u]")
{
	...
	REQUIRE(record_conflict_t::compute_conflict_all(values) == conflict_all_t::conflict);
}
...
{
	std::vector<std::string> values = { "A" };
	auto result = record_conflict_t::compute_conflict_this(values);
...
TEST_CASE("compute_conflict_all, empty merge column excluded yields no_conflict", "[u]")
{
	...
	REQUIRE(record_conflict_t::compute_conflict_all(filtered) == conflict_all_t::no_conflict);
}
...
TEST_CASE("compute_conflict_all, populated merge column participates in conflict", "[u]")
{
	...
	REQUIRE(record_conflict_t::compute_conflict_all(filtered) == conflict_all_t::override_benign);
}
...
TEST_CASE("compute_conflict_all, no merge column leaves values unchanged", "[u]")
{
	...
	REQUIRE(record_conflict_t::compute_conflict_all(filtered) == conflict_all_t::conflict);
}
...
TEST_CASE("compute_conflict_this, empty merge column excluded preserves statuses", "[u]")
{
	...
	auto result = record_conflict_t::compute_conflict_this(filtered);
