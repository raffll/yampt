...
TEST_CASE("dict_creator_t::make_dict, dial before info ordering", "[i]")
{
	...
	binary_file_io::write_text(native_content, native_path);
	binary_file_io::write_text(foreign_content, foreign_path);
...
TEST_CASE("dict_creator_t::make_dict, cell matching sequence", "[i]")
{
	...
	REQUIRE(ptr_exterior->status == status_t::translated);
	...
	REQUIRE(ptr_interior->status == status_t::translated);

	const auto * ptr_heuristic = cell_chapter.find_by_old_text("Old Mine");
	REQUIRE(ptr_heuristic != nullptr);
	REQUIRE(ptr_heuristic->status == status_t::missing);
}
