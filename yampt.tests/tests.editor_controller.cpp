...
#include "../yampt.translator/controller/editor_controller.hpp"
...
std::string create_test_dict(const tools_t::dict_t & data)
{
...
tools_t::dict_t make_simple_dict()
{
	tools_t::dict_t data;
...
TEST_CASE("editor_controller_t::commit, matching row returns success", "[i][qt]")
{
	...
	table_row_t row;
	row.type = tools_t::rec_type_t::cell;
	row.key_text = "key_c";
	...
	REQUIRE(result.status == tools_t::status_t::in_progress);
...
TEST_CASE("editor_controller_t::propagate, matches trimmed old_text", "[i][qt]")
{
	...
	const auto & chapter = doc.data().at(tools_t::rec_type_t::cell);
...
TEST_CASE("editor_controller_t::propagate, sets status to propagated", "[i][qt]")
{
	...
	const auto & chapter = doc.data().at(tools_t::rec_type_t::cell);
	REQUIRE(chapter.records[0].status == tools_t::status_t::propagated);
	REQUIRE(chapter.records[1].status == tools_t::status_t::propagated);
