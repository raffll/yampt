...
TEST_CASE("dict_creator_t::adapt_translation, size mismatch returns translation unchanged", "[u]")
{
	auto result = dict_creator_t::adapt_translation("AB", "ABC", "XYZ");
	REQUIRE(result == "XYZ");
}

TEST_CASE("dict_creator_t::is_interior_cell, interior flag set", "[u]")
{
	// DATA: flags byte 0 = 0x01 (interior), rest zeros
	std::string data(12, '\0');
	data[0] = '\x01';
	REQUIRE(dict_creator_t::is_interior_cell(data) == true);
}

TEST_CASE("dict_creator_t::is_interior_cell, exterior flag", "[u]")
{
	// DATA: flags byte 0 = 0x00 (exterior)
	std::string data(12, '\0');
	data[0] = '\x00';
	REQUIRE(dict_creator_t::is_interior_cell(data) == false);
}

TEST_CASE("dict_creator_t::is_interior_cell, interior with water", "[u]")
{
...
TEST_CASE("dict_creator_t::is_interior_cell, too short data", "[u]")
{
	REQUIRE(dict_creator_t::is_interior_cell("") == false);
	REQUIRE(dict_creator_t::is_interior_cell("AB") == false);
}

TEST_CASE("dict_creator_t::make_exterior_coord_key, positive coords", "[u]")
{
	...
	REQUIRE(dict_creator_t::make_exterior_coord_key(data) == "GRID[3,5]");
}

TEST_CASE("dict_creator_t::make_exterior_coord_key, negative coords", "[u]")
{
	std::string data(12, '\0');
	int32_t x = -7, y = -12;
	std::memcpy(data.data() + 4, &x, 4);
	std::memcpy(data.data() + 8, &y, 4);
	REQUIRE(dict_creator_t::make_exterior_coord_key(data) == "GRID[-7,-12]");
}

TEST_CASE("dict_creator_t::make_exterior_coord_key, zero coords", "[u]")
{
	std::string data(12, '\0');
	REQUIRE(dict_creator_t::make_exterior_coord_key(data) == "GRID[0,0]");
}

TEST_CASE("dict_creator_t::make_exterior_coord_key, too short data", "[u]")
{
	REQUIRE(dict_creator_t::make_exterior_coord_key("").empty());
	REQUIRE(dict_creator_t::make_exterior_coord_key("12345678").empty());
}

TEST_CASE("dict_creator_t::make_cell_key_text, produces 16-char hex string", "[u]")
{
	auto result = dict_creator_t::make_cell_key_text("test fingerprint");
...
TEST_CASE("dict_creator_t::make_cell_key_text, deterministic", "[u]")
{
	auto a = dict_creator_t::make_cell_key_text("Balmora fingerprint");
	auto b = dict_creator_t::make_cell_key_text("Balmora fingerprint");
	REQUIRE(a == b);
}
