...
void table_view_t::apply(table_build_result_t result, const std::string & file_path, dict_kind_t kind)
{
	...
	filter_tree_.set_display_mode(filter_tree_view_t::display_mode_t::full);
	set_enabled(true);

	status_bar_.set_document_open(true);
    ...
    const std::map<status_t, size_t> & total_status_counts)
{
	file_label_.setText(QString::fromStdString(file_path));
	filter_tree_.setEnabled(true);
	filter_tree_.set_display_mode(filter_tree_view_t::display_mode_t::all_only);
	status_bar_.set_document_open(true);
	set_enabled(true);
...
void table_view_t::clear()
{
	...
	status_bar_.set_document_open(false);
	set_enabled(false);
}
