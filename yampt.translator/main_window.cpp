...
#include "view/annotations_panel.hpp"
#include "view/book_preview.hpp"
...
void main_window_t::load_record(int row)
{
	...
	extra_sel_original_.annotations = build_highlight_selections(editor_panel_->original_view(), orig_highlights);
	...
	if (row_data->status == tools_t::status_t::adapted && !load_result.details.empty())
	{
		extra_sel_adapted_.adapted_diff =
		    editor_panel_->highlight_adapted_diff(row_data->new_text, load_result.details, false);
	}
	else if (row_data->status == tools_t::status_t::changed && !load_result.details.empty())
	{
		extra_sel_adapted_.adapted_diff =
...
void main_window_t::load_record_plain(const table_row_t * row_data)
{
	...
	if (row_data->status == tools_t::status_t::untranslated)
	...
	editor_panel_->translation_editor()->set_block_multiline(block_multiline);
}
...
void main_window_t::start_batch_translation(dict_document_t * dict_doc)
{
	...
	for (auto & [type, chapter] : data)
	{
		...
		{
			if (chapter.records[i].status == tools_t::status_t::untranslated && !chapter.records[i].old_text.empty())
