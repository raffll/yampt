...
- [x] Add a row of checkboxes/toggles in the toolbar, one per `rec_type_t` (CELL, DIAL, INFO, FNAM, TEXT, GMST, DESC, RNAM, INDX, BNAM, SCTX)
- [x] Implement `get_filtered_records()` in `editor_state_t` that respects the active type filter
- [x] Apply the filter to both panels simultaneously
- [x] Show record count per type next to each toggle

## Task 10: Text Search and Highlight

Implement text search with match highlighting.

- [x] Create `yampt.gui/search_manager.hpp` and `yampt.gui/search_manager.cpp`
- [x] Add a search input field in the toolbar
- [x] Implement `find_all()` that scans filtered records for matches in key or value text
- [x] Highlight matching substrings in the table cells using colored text spans
- [x] Implement `next_match()` / `prev_match()` with F3 / Shift+F3 keyboard shortcuts
- [x] Implement case-sensitive toggle
- [x] Implement "Go to Row" (Ctrl+G) popup that jumps to a specific row number

## Task 11: Find and Replace

Extend search with replace and replace-all functionality.

- [x] Add a Replace input field below the Search field in the toolbar
- [x] Implement `replace_current()` — replace the currently highlighted match in the value field
- [x] Implement `replace_all()` — replace all matches in all visible left-panel value fields, mark each as modified
- [x] Display replacement count in the status bar after replace-all
- [x] Add keyboard shortcuts (Ctrl+H to open replace, Enter to replace next)

## Task 12: Validation Manager

Implement byte-length validation with visual indicators on record rows.

- [x] Create `yampt.gui/validation_manager.hpp` and `yampt.gui/validation_manager.cpp`
- [x] Implement `validate()` with the rules: CELL > 63 = error, FNAM > 31 = error, RNAM > 32 = error, INFO > 512 = caution, INFO > 1024 = error
- [x] Display colored indicators in the table row (yellow for caution, red for error)
- [x] Show byte count tooltip on hover over the indicator
- [x] Auto-set status to `"has_errors"` when validation fails
...
- [x] Define pastel color palette for each status (untranslated, translated, auto_identical, auto_heuristic, validated, changed, has_errors)
- [x] Render the Status column cell with the appropriate background color
- [x] Implement the status summary bar below the toolbar showing counts per status in colored cells
- [x] Left-click on a summary cell → filter to show only that status
- [x] Right-click on a summary cell → hide that status (inverse filter)
- [x] Support combining multiple status filters
- [x] Right-click context menu on records to manually change status
- [x] Allow promoting auto_identical / auto_heuristic to validated

## Task 14: Unsaved Changes Indicator and Close Prompt

Implement the dirty-state indicator and save-on-close dialog.

- [x] Append `" *"` to the window title when `has_unsaved_changes()` is true
- [x] On close attempt (SDL_QUIT or File > Exit), show a modal dialog: Save / Discard / Cancel
- [x] Wire Save to `save_user_dict()`, Discard to quit without saving, Cancel to abort close

## Task 15: History Manager

Implement per-record change history with persistence.

- [x] Create `yampt.gui/history_manager.hpp` and `yampt.gui/history_manager.cpp`
- [x] Record each value change with ISO 8601 timestamp
- [x] Implement `get_history()` returning the list of previous values for a record
- [x] Implement `revert()` to restore a previous value
- [x] Implement `load_from_file()` / `save_to_file()` for the `.history.json` sidecar
- [x] Display a "modified this session" indicator (dot or icon) on changed records

## Task 16: History View Panel

Render the history panel UI.

- [x] Add a toggleable history panel (View menu or keyboard shortcut)
- [x] When a record is selected, display its change history as a scrollable list
- [x] Each entry shows the value text (truncated) and timestamp
- [x] Double-click or "Revert" button restores that value and records the revert as a new history entry

## Task 17: Syntax Highlighter — MWScript (SCTX)

Implement syntax highlighting for SCTX records.

- [x] Create `yampt.gui/syntax_highlighter.hpp` and `yampt.gui/syntax_highlighter.cpp`
- [x] Implement `tokenize()` for SCTX: identify MWScript functions (MessageBox, Say, Journal, Choice, AddTopic, GetPCCell, etc.), comments (`;` lines), and string literals (`"..."`)
- [x] Use `string::find`-based tokenization (no regex per design rules)
- [x] Render tokenized text with colored spans in the table cell and inline editor

## Task 18: Syntax Highlighter — Book HTML Tags (TEXT)

Implement tag highlighting for TEXT records.

- [x] Extend `syntax_highlighter_t::tokenize()` for TEXT type: identify `<DIV>`, `</DIV>`, `<FONT>`, `</FONT>`, `<BR>`, `<P>`, `<IMG>`, `<B>` and attributes
- [x] Render tags in a muted/dimmed color so translatable content stands out
- [x] Maintain highlighting in the inline multi-line editor during editing

## Task 19: Annotation Manager — DIAL Topics

Implement DIAL topic detection and annotation in INFO records.

- [x] Create `yampt.gui/annotation_manager.hpp` and `yampt.gui/annotation_manager.cpp`
- [x] On dictionary load, build a sorted list of DIAL keys (longest first for greedy matching)
- [x] Implement `annotate()` for INFO records: case-insensitive substring search with word-boundary check (preceding char not alphabetic)
- [x] Highlight matched topics in Morrowind hyperlink blue within the table cell
- [x] Show tooltip with translated DIAL name on hover

## Task 20: Annotation Manager — Glossary and Speaker

Extend annotations with glossary terms and NPC gender flags.

- [x] Support loading a Glossary dictionary (FNAM-derived names) as additional reference
- [x] Support loading an NPC_FLAG dictionary for speaker gender
- [x] Add glossary term matching (same logic as DIAL topics)
- [x] Display annotations panel with sections: Hyperlinks, Glossary, Speaker
- [x] Click on a proposal → insert translated term at cursor or copy to clipboard
- [x] Update annotations dynamically when dictionaries are modified

## Task 21: Enchanted Item Indicator for FNAM Records

Show enchantment status for weapon/armor/clothing FNAM records.

- [x] Support loading an enchantment reference dictionary (or derive from ESM data)
- [x] When an FNAM record belongs to an enchanted item, display "⚡" indicator in the row
- [x] Show enchantment name in the annotations panel when the record is selected

## Task 22: Base Dictionary Manager

Implement multi-dictionary merge and auto-translation.

- [x] Create `yampt.gui/base_dict_manager.hpp` and `yampt.gui/base_dict_manager.cpp`
- [x] Implement `set_paths()` / `reload()` using `dict_merger_t` with first-wins semantics
