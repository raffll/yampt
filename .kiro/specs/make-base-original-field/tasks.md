...
- [ ] 1. Rename RecordEntry fields and update JSON keys
...
- [ ] 3. Create unified `insertEntry` function
  - [ ] 3.1 Implement `insertEntry(id, old_text, new_text, type)` in DictCreator
  ...
  - [ ] 3.2 Update all callers to use `insertEntry`
...
- [ ] 5. Build lookup indexes from `esm_ref`
  - [ ] 5.1 Add `buildIndexes()` method and index member variables
...
- [ ] 6. Refactor Pattern 2 extractors — single loop, no `is_make_mode` branch
  - [ ] 6.1 Refactor `makeDictGMST`
...
- [ ] 8. Refactor Pattern 1 extractors — make-mode only, remove base-mode branch
  - [ ] 8.1 Simplify `makeDictCELL` to make-mode only
  ...
  - [ ] 8.2 Simplify `makeDictCELL_Default` to make-mode only
    - Remove base-mode branch, keep make-mode loop
    - Base mode handled by `makeDictCELL_Unordered_Default`
    - _Requirements: 15.1, 22.4_

  - [ ] 8.3 Simplify `makeDictCELL_REGN` to make-mode only
    - Remove base-mode branch, keep make-mode loop
    - Base mode handled by `makeDictCELL_Unordered_REGN`
    - _Requirements: 15.1, 22.4_

  - [ ] 8.4 Simplify `makeDictDIAL` to make-mode only
    - Remove base-mode branch, keep make-mode loop
    - Base mode handled by `makeDictDIAL_Unordered`
    - _Requirements: 8.2, 15.1, 22.4_

  - [ ] 8.5 Simplify `makeDictScript` to make-mode only
    - Remove base-mode branch, keep make-mode loop
    - Base mode handled by `makeDictScript_Unordered`
    - _Requirements: 7.1, 15.1, 22.4_

- [ ] 9. Update unordered variants to use `esm_ref` and pass `old_text`
  - [ ] 9.1 Update `makeDictCELL_Unordered` — replace `esm_ext` with `esm_ref`, pass foreign cell name as `old_text` to `insertEntry`
    - _Requirements: 10.1, 21.3_

  - [ ] 9.2 Update `makeDictCELL_Unordered_Default` — replace `esm_ext` with `esm_ref`, pass foreign default cell name as `old_text`
    - _Requirements: 10.4, 21.3_

  - [ ] 9.3 Update `makeDictCELL_Unordered_REGN` — replace `esm_ext` with `esm_ref`, pass foreign region FNAM as `old_text`
    - _Requirements: 10.5, 21.3_

  - [ ] 9.4 Update `makeDictDIAL_Unordered` — replace `esm_ext` with `esm_ref`, pass foreign topic name as `old_text`
    - _Requirements: 10.2, 21.3_

  - [ ] 9.5 Update `makeDictScript_Unordered` — replace `esm_ext` with `esm_ref`, pass foreign script key+message as `old_text`
    - _Requirements: 10.3, 21.3_

  - [ ] 9.6 Update `makeDictCELL_Unordered_AddMissing` and `makeDictDIAL_Unordered_AddMissing` — replace `esm_ext` with `esm_ref`
    - _Requirements: 10.1, 10.2, 21.3_

  - [ ] 9.7 Update all pattern/helper functions — replace `esm_ext` with `esm_ref`
    - `makeDictCELL_Unordered_PatternsExt`, `makeDictDIAL_Unordered_PatternsExt`, `makeDict_Unordered_PatternsExt`
    - _Requirements: 21.1, 21.2, 21.4_

  - [ ] 9.8 Update `makeDictFNAM_Glossary` — replace `esm_ext` with `esm_ref`, pass foreign FNAM as `old_text`
    - _Requirements: 12.1, 12.2, 21.3_

- [ ] 10. Checkpoint
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 11. Unify orchestrators into single `makeDict()`
  - [ ] 11.1 Replace `makeDictForMake()` and `makeDictForBase()` with `makeDict()`
...
- [ ] 12. Remove dead code and update declarations
  - [ ] 12.1 Remove `isSameOrder()`, `makeDictForMake()`, `makeDictForBase()` declarations from header
    - Remove `same_order` member variable if present
    - _Requirements: 22.1, 18.1_

  - [ ] 12.2 Remove any remaining old function bodies (`insertRecord`, `insertRecordToDict`) if not already removed in task 3.2
    - _Requirements: 14.1_
...
- [ ] 14. Update tests
  - [ ] 14.1 Update tests.dictcreator.cpp for new `insertEntry` behavior
  ...
  - [ ] 14.2 Update remaining test files for renamed fields and JSON keys
